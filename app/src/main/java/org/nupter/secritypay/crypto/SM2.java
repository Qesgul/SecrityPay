package org.nupter.secritypay.crypto;
...
public class SM2 {
	 ...
	 * @param publicKey
	 */
	public void exportPublicKey(ECPoint publicKey, String SavePath) {
		try {
			File path = new File(SavePath);
			String filepath = SavePath +"/publickey.pem";
	 ...
	 * @param privateKey
	 */
	public void exportPrivateKey(BigInteger privateKey, String SavePath) {
		try {
			File path = new File(SavePath);
			String filepath = SavePath +"/privatekey.pem";
			File file = new File(filepath);
			if (!path.exists()) {
				path.mkdirs();
			}
